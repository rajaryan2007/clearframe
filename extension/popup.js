document.getElementById('analyze-btn').addEventListener('click', async () => {
  ...
  chrome.scripting.executeScript({
    ...
    try {
      // Internal API call to ClearFrame (assumes app is running on localhost:3000)
      ...
      // Since analysis is async, we poll the history API
      // Note: This requires the user to be logged in on the web app
      let attempts = 0;
      const poll = setInterval(async () => {
        ...
        const latest = historyData.find(item => item.input === selectedText);
        if (latest) {
          clearInterval(poll);
          showResult(latest);
        }
